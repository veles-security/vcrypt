...
var ErrClosed = errors.New("random source is closed")
...
func New(id string, keyType KeyType, lifetime time.Duration) (*Source, error) {
	...
	return &Source{id: id, keyType: keyType, lifetime: lifetime, ctx: lifecycleContext, cancel: cancel, keys: make(map[int64]key.KeyCandidate), wake: make(chan struct{}, 1), now: time.Now}, nil
}
...
func (s *Source) Load(ctx context.Context) ([]key.KeyCandidate, error) {
	if ctx == nil {
		return nil, fmt.Errorf("load random source: nil context")
	}
...
func (s *Source) startLocked() {
	if s.closed || s.started || s.callback == nil || s.base.IsZero() {
		return
	}
	s.started = true
	go s.watch()
